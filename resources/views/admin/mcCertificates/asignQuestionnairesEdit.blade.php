...
<div class="panel panel-flat">
    ...
    <div class="panel-body">
        {!! Form::open(array('route' => 'update_mc_certificates_mc_questionnaires', 'name' => 'Form', 'id' => 'mc_certificate-questionnaires', 'method'=>'post', 'class'=>'form-horizontal form-validate-jquery')) !!}
                ...
                <div class="col-lg-5">
                    <select class="form-control" multiple="multiple" id="mc_questionnaire_id" name="mc_questionnaire_id[]" required="required">
                        @foreach($mcQuestionnaires as $questionnaries)
                            @if($questionnaries->selected == 1)
                                <option value="{{ $questionnaries->id }}" selected>{{ $questionnaries->title }}</option>      
                            @else
